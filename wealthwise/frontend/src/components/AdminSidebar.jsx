...
export default function AdminSidebar({ active, setActive, collapsed, onToggle }) {
  const navigate = useNavigate();
  const [showLogoutConfirm, setShowLogoutConfirm] = useState(false);

  const nav = [
    { id: "dashboard", label: "Dashboard", icon: <Ico.Dash /> },
    { id: "users", label: "User Management", icon: <Ico.Profile /> },
    { id: "add-funds", label: "Add Mutual Funds", icon: <Ico.Funds /> },
    { id: "alerts", label: "Send Alerts", icon: <Ico.Bell /> },
  ];
      ...
      <div className="sidebar__logo">
        ...
        {!collapsed && (
          <span className="sidebar__logo-text">
            Wealth<em>Wise</em>{" "}
            <span style={{ fontSize: "10px", color: "var(--green)", verticalAlign: "middle", letterSpacing: "1px", fontFamily: "var(--font-m)" }}>ADMIN</span>
          </span>
        )}
        <button
      ...
      {showLogoutConfirm && (
        <div className="logout-modal-overlay" onClick={() => setShowLogoutConfirm(false)}>
          <div className="logout-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="logout-modal__msg">Are you sure you want to logout from admin?</p>
